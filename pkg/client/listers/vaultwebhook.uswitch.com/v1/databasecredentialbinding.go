...
type DatabaseCredentialBindingLister interface {
	...
	List(selector labels.Selector) (ret []*v1.DatabaseCredentialBinding, err error)
...
// List lists all DatabaseCredentialBindings in the indexer.
func (s *databaseCredentialBindingLister) List(selector labels.Selector) (ret []*v1.DatabaseCredentialBinding, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*v1.DatabaseCredentialBinding))
	})
	return ret, err
}
...
type DatabaseCredentialBindingNamespaceLister interface {
	...
	// Objects returned here must be treated as read-only.
	Get(name string) (*v1.DatabaseCredentialBinding, error)
...
// List lists all DatabaseCredentialBindings in the indexer for a given namespace.
func (s databaseCredentialBindingNamespaceLister) List(selector labels.Selector) (ret []*v1.DatabaseCredentialBinding, err error) {
...
// Get retrieves the DatabaseCredentialBinding from the indexer for a given namespace and name.
func (s databaseCredentialBindingNamespaceLister) Get(name string) (*v1.DatabaseCredentialBinding, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(v1.Resource("databasecredentialbinding"), name)
	}
	return obj.(*v1.DatabaseCredentialBinding), nil
}
